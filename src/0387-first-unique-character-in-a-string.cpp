...
int first_unique(string const& s) {
  ...
  // an ASCII lowercase letter.  When a character is seen, the array is updated
  // to reflect the index where it was seen. If a character is subsequently
  // seen, the value is updated to INT_MAX - 1. Then finding the first
  // non-repeating character consists of finding the minimum value in the array.
